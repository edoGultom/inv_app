...
function ModalRemote(modalId) {
  ...
  this.setTitle = function (title) {
    ...
    // $(this.header).append('button.btn-close').remove();
    $(this.header).append(
  ...
  function errorRemoteResponse(response) {
    ...
    if (kode == 500) {
      content =
        "Ada masalah saat mencoba terhubung ke database, <br> Silahkan coba beberapa saat lagi atau hubungi pihak administrator";
    } else {
      $.ajax({
        url: "/handle-error-ajax/index?kodestatus=" + kode,
        method: "get",
        async: false,
